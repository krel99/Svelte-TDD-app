...
// const removeAnsiColorCodes = (string) => string.replace(/\x1b\[[0-9;]+m/g, '');
const removeAnsiColorCodes = (string) => string.replace(/\\x1b\[[0-9;]+m/g, '');

class PlaywrightWorld {
	async launchServer() {
		console.log('launching server');
		this.serverProcess = childProcess.spawn(config.webServer.command, [], {
			shell: true,
		...
		this.baseUrl = await new Promise((resolve) => {
			this.serverProcess.stdout.on('data', (data) => {
				...
				let match = text.match(/http[s]?:\/\/[a-z]+:[0-9]+\//);
				if (match) {
					resolve(match[0]);
				}
			});
