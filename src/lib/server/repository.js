...
const validate = ({ name, dateOfBirth }) => {
	if (empty(name)) {
		return { error: 'Please provide a name.' };
	}

	if (invaliddateOfBirth(dateOfBirth)) {
		return {
...
const empty = (value) => value === undefined || value === null || value.trim() === '';

const invaliddateOfBirth = (dateOfBirth) => isNaN(Date.parse(dateOfBirth));
